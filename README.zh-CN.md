...
本项目是基于开源项目 **CodexScope** 的本地衍生和个人改造版本。原项目由 **JUk1-GH** 创建并发布：

- 原始仓库：[JUk1-GH/CodexScope](https://github.com/JUk1-GH/CodexScope)
- 原作者/维护者：[JUk1-GH](https://github.com/JUk1-GH)
- 许可证：MIT，详见 [LICENSE](LICENSE)

本版本保留原项目署名和许可证，并在此基础上进行本地修改；本版本不是上游仓库本身。
...
如果要使用实时面板，在 Windows 上双击 `windows/open-dashboard.cmd`。启动器会运行 Rust 本地服务 `http://127.0.0.1:4173/`，打开面板并监控本地 Codex 会话目录。会话日志发生变化后，服务会重新生成本地导出；开启实时模式时，页面会自动刷新并保留当前滚动位置。
...
Rust 服务会优先使用现有的预编译 Go 生成器；如果没有预编译生成器，则在安装 Go 时调用现有的 Go 生成器。如果两者都不存在，页面仍然可以使用内置示例数据，但无法生成最新的本地用量导出。
...
- `live.js`：浏览器端 SSE 客户端、实时刷新开关、连接状态和手动刷新。
- `live-server/`：Rust 本地服务，负责静态文件、日志变化监控和 SSE 更新。
