  ...
  for (const [width, height] of viewports) {
    ...
    const report = await page.evaluate(() => {
      ...
      return {
        ...
        liveState: {
          hasStatus: !!document.querySelector("#liveStatus"),
          hasToggle: !!document.querySelector("#liveToggle"),
    ...
    const interactions = await page.evaluate(async () => {
      ...
      const beforeSessionSearch = count("#sessionList .session-row");
      const beforeModelSearch = count("#modelList .model-row");
      if (sessionSearch) {
        sessionSearch.value = "zzzz-no-match";
        sessionSearch.dispatchEvent(new Event("input", { bubbles: true }));
      }
      if (modelSearch) {
        modelSearch.value = "zzzz-no-match";
        modelSearch.dispatchEvent(new Event("input", { bubbles: true }));
      }
      const afterSessionSearch = count("#sessionList .session-row");
      const afterModelSearch = count("#modelList .model-row");
      if (sessionSearch) {
        sessionSearch.value = "";
        sessionSearch.dispatchEvent(new Event("input", { bubbles: true }));
      }
      ...
      return {
        ...
        sessionSearchFilters: !sessionSearch || afterSessionSearch < beforeSessionSearch,
        modelSearchFilters: !modelSearch || afterModelSearch < beforeModelSearch,
        defaultLinearScale,
        trendMultiSeries,
    ...
    if (!interactions.sessionSearchFilters) issues.push("session search does not filter rows");
    if (!interactions.modelSearchFilters) issues.push("model search does not filter rows");
